...
import {
  ...
  ArrowRight, ShieldCheck, X, ExternalLink, CheckCircle2, MessageCircle
} from 'lucide-react';
        ...
        <div className="p-7">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <a
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-[#25D366] hover:bg-[#1ebe5c] text-white font-semibold rounded-2xl transition-all text-sm"
            >
            ...
            <Link
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-gray-900 hover:bg-teal-700 text-white font-semibold rounded-2xl transition-all duration-200 text-sm group/btn"
            >
...
export default function ServicePage() {
  ...
  useState(() => {
    if (service) {
      document.title = `${service.name} | Head Held High — Bengaluru`;
      const meta = document.querySelector('meta[name="description"]');
      if (meta) meta.setAttribute('content', service.description.slice(0, 160));
    }
  });
  ...
  return (
    <>
      ...
      {/* Sub-Services Section — only rendered when subServices exist (e.g. Turnkey) */}
      {service.subServices && service.subServices.length > 0 && (
        <section className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-12">
              <span className="inline-block px-4 py-1 bg-orange-50 text-orange-600 text-sm font-semibold rounded-full mb-4 border border-orange-200">
                Specialised Sub-Services
              </span>
              <h2 className="text-3xl sm:text-4xl font-bold text-gray-900">
                Included Within {service.name}
              </h2>
            </div>
                ...
                <div className="grid lg:grid-cols-2">
                  ...
                  <div className="relative h-64 lg:h-auto min-h-[320px]">
                    ...
                    <div className="absolute top-6 left-6">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r ${service.gradient} text-white text-xs font-bold rounded-full shadow-lg`}>
                        Sub-Service
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-8 lg:p-10 flex flex-col justify-center">
                    ...
                    <p className="text-gray-500 leading-relaxed mb-8">{sub.description}</p>
                    ...
                    <div className="mt-8">
                      <Link
                        to="/contact"
                        state={{ service: service.name, subService: sub.title }}
                        className={`inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r ${service.gradient} text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 text-sm group`}
                      >
                        Enquire About {sub.title}
            ...
            <div className={`grid gap-8 ${vendors.length === 1 ? 'max-w-lg mx-auto' : 'sm:grid-cols-2 lg:grid-cols-3'}`}>
              {vendors.map((vendor) => {
                ...
                return (
                  <div
                    key={vendor.id}
                    className="group relative bg-white rounded-2xl border border-gray-200 hover:border-teal-300 shadow-sm hover:shadow-xl hover:shadow-teal-500/10 transition-all duration-300 sm:hover:-translate-y-1.5 flex flex-col overflow-hidden cursor-pointer active:scale-[0.99]"
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="flex items-start gap-4 mb-5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {vendor.badge === 'In-House HOC' ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-teal-600 text-white text-xs font-bold rounded-full shadow-sm shadow-teal-500/30">
                      ...
                      <div className="mt-auto flex gap-2">
                        <a
                          href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Hi, I'm interested in ${vendor.name}'s services.`)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-[#25D366] hover:bg-[#1ebe5c] text-white font-semibold rounded-xl transition-all duration-200 text-sm"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <MessageCircle className="w-4 h-4" />
                          WhatsApp
                        ...
                        <button
                          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gray-900 hover:bg-teal-700 text-white font-semibold rounded-xl transition-all duration-200 text-sm group/btn"
